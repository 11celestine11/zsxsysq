...
interface WorldbookOverlayProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function WorldbookOverlay({ isOpen, onClose, worldbooks, setWorldbooks, title }: WorldbookOverlayProps) {
  ...
  const handleAdd = () => {
    if (newTitle && newContent) {
      if (editingId) {
        setWorldbooks(prev => prev.map(w => w.id === editingId ? { ...w, title: newTitle, content: newContent } : w));
      } else {
        setWorldbooks(prev => [...prev, { id: Date.now().toString(), title: newTitle, content: newContent }]);
      }
      resetForm();
    }
  };
  ...
  const startEdit = (item: Worldbook) => {
